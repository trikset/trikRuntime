	...
	, mLoggingHelper(new LoggingHelper(Paths::logsPath()))
{
	...
	mCommandLineParser.addOption("c", "config-path"
	                             , QObject::tr(
					     "Path to a directory where all configs for TRIK runtime are stored. Config files are\n"
	...
	mCommandLineParser.addOption("d", "coredump-path"
	                             , QObject::tr(
					     "Path to a directory where core dump will be saved in case of creation.")
	                             );
