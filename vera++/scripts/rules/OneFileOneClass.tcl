...
proc checkClassNumber { fileName } {
  ...
  foreach line [getAllLines $fileName] {
    if {[regexp -nocase {^[ \t]*class [a-z0-9]*} $line] && ![regexp -nocase {^[ \t]*class [a-z0-9]*;} $line] && $openedBraces == 0} {
      incr classesDeclared
      if {$classesDeclared > 1} { 
        report $fileName $lineCount "Multiple class definitions in one file" 
      }
    }
