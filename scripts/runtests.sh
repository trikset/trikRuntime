...
for t in "$@"
  do
    ...
    ASAN_OPTIONS="$( [[ $TRAVIS_OS_NAME == linux ]] && echo 'detect_leaks=1:disable_coredump=0:' || :)detect_stack_use_after_return=1:fast_unwind_on_malloc=0:symbolize=1:use_sigaltstack=0" \
